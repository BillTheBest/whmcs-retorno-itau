...
echo '<b>Sistema para processamento e baixa de arquivos de retorno do banco Itaú</b><br>';
...
if ($_POST && $_FILES['arquivo']['error'] != 0) {
	...
	foreach ($arquivo as $dados) {
		...
		if ($i > 0 && $i < count($arquivo)-1) {
			...
			$documento = intval(substr($dados,62,8));
			$valor     = intval(substr($dados,	152, 13));
	...
	if($i >= 2) {
		...
		echo "<br><table style='background: #f5f5f5; vertical-align:top; border:0px; padding: 10px;'><tr><td colspan=5><b>Detalhes do Processamento:</b></td></tr><tr><td width=100><b>Invoice</b></td><td><b>Cliente</b></td><td width=100  style='padding-left:10px;'><b>Valor</b></td><td width=100><b>Tarifa</b></td><td><b>Status</b></td><td><b>Data</b></td></tr>";
		foreach ($retorno as $dados) {
			echo "<tr style='padding:3px;'><td>" . $dados["documento"] . "</td><td>" . $dados["cliente"] . "</td><td style='padding-left:10px;'>" . $dados["valor"] . "</td><td>" . $dados["tarifa"] . "</td><td>" . $dados["status"] . "</td><td>" . $dados["data"] . "</td></tr>";
		}
		echo "</table>";
